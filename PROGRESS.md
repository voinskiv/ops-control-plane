...
- [x] SLICE-008: Manager auth — magic link, session, login + authenticated shell
...
- [x] SLICE-015: Day-pack read + browser-first Heute board
      ...
      Follow-up status: TIMEZONE issue #51 centralizes §15 workspace-time formatting at the request boundary, passes the timezone to the client board as a primitive, and adds the app-scoped raw-date-format lint guard.
...
- First-owner bootstrap: seed uses a simulated owner context for the initial person.create (SLICE-011 one-liner); real workspace signup needs a first-owner contract (platform-authorized first person or atomic workspace.create+owner) — resolve by DEC before SLICE-041.
