...
| execution_records | window_id, kind enum(presence, coverage_confirm, output, service_confirmation, note), subject_person_id?, qty numeric?, unit?, started_at?, ended_at?, occurred_at, received_at, captured_by_actor, device_id? (reserved; always NULL in v1), supersedes_id?, client_key, status | window | §4 | workspace_id |
...
| record.capture | S, M | window_id, kind, qty/times, client_key | proposal_gated | device_id reserved/NULL in v1, occurred_at vs received_at skew, auto-verified marker (F32) | client uuid (= client_key) |
