...
- 2026-07-12 SLICE-011: the seed reuses `Kernel.dispatch` with a deterministic simulated owner context only for the first `person.create` bootstrap (matching the existing kernel-test actor-context pattern); every later person/client/site invocation, including human-only `site.activate`, dispatches as the returned seeded owner, and pinned `tsx` exists only to run the TypeScript migration/seed entrypoints.
