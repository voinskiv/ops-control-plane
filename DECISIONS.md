...
- 2026-07-15 FIX-040 (DEC-028/029): the local-only bootstrap first validates `SUPABASE_URL` as localhost/127.0.0.1/::1, then reads the seeded active owner, replays its existing self-invite as that owner through the public kernel, ensures a confirmed local Auth user with the documented `local-dev-password`, and dispatches the unchanged internal link action.
